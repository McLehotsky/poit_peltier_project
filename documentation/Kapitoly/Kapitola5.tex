...
Stav \textit{Default} slúži na realtime sledovanie a ovládanie systému. Obsahuje grafy teploty, PWM pumpy a PWM Peltierovho článku, 
tlačidlá \textit{Connect}, \textit{Start}, \textit{Stop}, prepínanie módov a vstupy 
pre manuálne PWM hodnoty. 

\begin{table}[H]
\centering
\begin{tabular}{llll}
\toprule
\textbf{Widget (ID)} & \textbf{Sledovaná veličina} & \textbf{Časové okno} & \textbf{Aktualizácia} \\
\midrule
Teplota [C°] (\texttt{w\_teplota}) & \texttt{temperature}, \texttt{setpoint} & 5 minút (300 s) & 1 s \\
Pumpa (\texttt{w\_pumpa})          & \texttt{pump\_pwm}                     & 1 minúta (60 s) & 1 s \\
PET (\texttt{w\_pet})              & \texttt{tec\_pwm}                      & 1 minúta (60 s) & 1 s \\
\bottomrule
\end{tabular}
\caption{Real-time vizualizačné widgety}
\end{table}
...
\begin{tabular}{llll}
\toprule
\textbf{Widget (ID)} & \textbf{RPC metóda} & \textbf{Parameter} & \textbf{Akcia} \\
\midrule
Connect (\texttt{w\_connect}) & \texttt{setConnected} & \texttt{true}  & Aktivuje pripojenie systému \\
Start (\texttt{w\_start})     & \texttt{setRunning}   & \texttt{true}  & Spustí meranie / reguláciu \\
Stop (\texttt{w\_stop})       & \texttt{setRunning}   & \texttt{false} & Zastaví meranie a uloží session \\
\bottomrule
\end{tabular}
\caption{Ovládacie tlačidlá --- RPC príkazy}
\end{table}
...
\begin{tabular}{lll}
\toprule
\textbf{Widget (ID)} & \textbf{Zdieľaný atribút} & \textbf{Hodnota} \\
\midrule
Mod 1 (\texttt{w\_mod1}) & \texttt{control\_mode} & 1 \\
Mod 2 (\texttt{w\_mod2}) & \texttt{control\_mode} & 2 \\
Mod 3 (\texttt{w\_mod3}) & \texttt{control\_mode} & 3 \\
\bottomrule
\end{tabular}
\caption{Tlačidlá módov --- zdieľané atribúty}
\end{table}
...
\begin{tabular}{llll}
\toprule
\textbf{Widget (ID)} & \textbf{Zdieľaný atribút} & \textbf{Typ} & \textbf{Rozsah} \\
\midrule
Ovládanie: Pump PWM (\texttt{w\_pump\_ctrl}) & \texttt{target\_pump\_pwm} & integer & 0 -- 255 \\
Ovládanie: TEC PWM (\texttt{w\_tec\_ctrl})   & \texttt{target\_tec\_pwm}  & integer & 0 -- 255 \\
\bottomrule
\end{tabular}
\caption{Vstupné polia pre manuálne ovládanie}
\end{table}

 \begin{itemize}
  \item \textbf{Error} (\texttt{w\_error}): Zobrazuje aktuálnu regulačnú odchýlku (setpoint $-$ teplota) ako read-only hodnotu z telemetrie.
  \item \textbf{Terminal} (\texttt{w\_terminal}): Zobrazuje diagnostický reťazec \texttt{terminal}, ktorý server aktualizuje pri každom publikovaní telemetrie vo formáte: \texttt{T=XX.X°C | Err=X.XX | RUN/STOP | Mode=X}.
\end{itemize}
